...
object CS143Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    var expressionsR = expressions.reverse


  for(x <- expressionsR) {
    if(udf == null) {
        if (x.isInstanceOf[ScalaUdf]) {
          udf = x.asInstanceOf[ScalaUdf]
        }
      }
    }
    udf
    ...
    new Iterator[Row] {
      ...
      def next() = {
        ...
        if(input.hasNext) {
          ...
          var key: Row = cacheKeyProjection(row)
          var preUdf: Row = preUdfProjection(row)
          var postUdf: Row = postUdfProjection(row)
          var udfEval: Row = udfProject(row)
          if (cache.containsKey(key)) {
            udfEval = cache.get(key) //don't eval if seen before, i.e. in cache
          }
          else
          {
            cache.put(key, udfEval) //if input has not be seen before
          }

          //create a row from a sequence of values
          //Row.fromSeq(Seq(preUdf, udfEval, postUdf)) 
          var value = preUdf ++ udfEval ++ postUdf
          Row.fromSeq(value)
        }
        else
          null
...
object AggregateIteratorGenerator {
  def apply(resultExpressions: Seq[Expression],
    ...
    new Iterator[Row] {
      ...
      def next() = {
                /* IMPLEMENT THIS METHOD */
        val (groupdata, buffer) = input.next()
        val aggregateResults = buffer.eval()
        postAggregateProjection(new JoinedRow(new GenericRow(Array(aggregateResults)), groupdata))
