...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    var cacheIterator: Iterator[Row] = null //a new iterator initially pointing to null
    var diskHashed: DiskHashedRelation = DiskHashedRelation(input, keyGenerator) //partitions whole input to disk, key generator provide, size and block size are default
    var diskHashedIterator: Iterator[DiskPartition] = diskHashed.getIterator() //iterator for partitions

    new Iterator[Row] {
      def hasNext() = {
        /* IMPLEMENT THIS METHOD */
        var hasNextIt = false
        if (cacheIterator != null) //check if null first
          {
            hasNextIt = cacheIterator.hasNext //check if it has next
            if (!hasNextIt) { //if it doesn't, fetches next partition
              hasNextIt = fetchNextPartition() //has next is true if next partition returns true, no more partitions means no more next
            }
          }
        else //initial state
        {
          hasNextIt = fetchNextPartition()
        }
        hasNextIt
      }
      ...
      def next() = {
        ...
        if(cacheIterator != null && cacheIterator.hasNext)
          cacheIterator.next()
        else //if this returns false it means no more partitions
          null
      }
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        if (diskHashedIterator.hasNext) //if there is another partition
        {
          var returnValue = false
          diskPartition = diskHashedIterator.next() //next partition
          var data: Iterator[Row] = diskPartition.getData() //get data method returns iterator, get data from next partition
          if (data.hasNext) { //if the iterator doesn't have next no more partitions to iterator over
            cacheIterator = CS143Utils.generateCachingIterator(projectList, child.output)(data) //reassigned cache iterator to iterating the next partition
            returnValue = true
          }
          returnValue
        }
