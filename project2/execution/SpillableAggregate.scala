    ...
    new Iterator[Row] {
      ...
      /**
        ...
        */
        ...
        /*1) drain the input iterator (data) into the aggregation table;
         ...
         and 3) use the Iterator inside generateIterator as external interface to access and drive the aggregate iterator.*/
      ...
      private def aggregate(): Iterator[Row] = {
        ...
        while (data.hasNext) {
          ...
          var aggregateFunction = currentAggregationTable(projection) //draining
          if(aggregateFunction == null) {
            aggregateFunction = newAggregatorInstance()
            currentAggregationTable.update(projection, aggregateFunction)
          }
          if(aggregateFunction != null) {
            aggregateFunction.update(row)
          }
        }

        var aggregateInput = List(aggregatorSchema) ++ namedGroups.map(_._2)
        AggregateIteratorGenerator(resultExpression, aggregateInput)(currentAggregationTable.iterator)
